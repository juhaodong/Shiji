...
import modules.physic.PlatformColors

@Composable
fun AadenMenuTheme(
    currentTheme: CurrentTheme, content: @Composable () -> Unit
) {
    val colorScheme = rememberDynamicColorScheme(
        seedColor = currentTheme.currentColor,
        isDark = currentTheme.darkMode,
        style = currentTheme.currentStyle,
        isAmoled = true,
    )

    PlatformColors(currentTheme.darkMode)




    MaterialTheme(
        colorScheme = colorScheme, typography = getType(), content = content
    )
}


data class CurrentTheme(
    val darkMode: Boolean, val currentColor: Color, val currentStyle: PaletteStyle
)
